...
const SearchBar = ({
  ...
  <Box sx={{display: 'flex', flexDirection: "column", gap: 2}}>
    <Box aria-label="line-1" sx={{gap: 2}}>
      <Box sx={{display: "flex", gap: 2, flexDirection: { xs: 'column', sm: 'column', md: 'row', lg: 'row'}}}>

        <Box sx={{ display: "flex", flexDirection: {xs: 'column', sm: 'row', md: 'row', lg: 'row'}, gap: 2, maxWidth: '100%', flex: 1}}>
          <Box sx={{ flex: 1 }}>
          <TextField
            label="Title"
            variant="outlined"
          ...
          <Box sx={{ flex: 1 }}>
            <TextField
              label="Code Template Data"
              variant="outlined"
              size="small"
              value={search.codeTemplate}
              onChange={(e) => setSearch({...search, codeTemplate: e.target.value})}
              onKeyDown={onKeyDown}
              sx={{width: "100%"}}
            />
          </Box>
        ...
        <Box sx={{ display: "flex", gap: 2, flexDirection: "row", width: {sm: "100%", xs: "100%", md: "auto"} }} >
          <Box sx={{}}>
            <TextField
              label="Tags"
              variant="outlined"
          ...
          <Box aria-label='search-button' sx={{flexGrow: 1}}>
            <Button
              variant="contained"
              onClick={onClick}
              sx={{whiteSpace: 'nowrap', height: "100%", width: { xs: '100%', sm: '100%', md: 'auto'}}}
            >
    ...
    <Box aria-label="line-2" sx={{display: 'flex', flexDirection: "row"}}>
      <Box sx={{ display: 'flex', gap: 1, flexDirection: 'row' }}>
        ...
        <Box aria-label="pagination-box" sx={{ width: { xs: '100%', sm: '100%', md: 'auto'}}}>
          <FormControl sx={{ minWidth: '110px',  width: { xs: 'auto', sm: '100%', md: 'auto'} }}>
      ...
      <Box aria-label="tag-chip" sx={{ display: 'flex', flexWrap: 'wrap', width: '100%', maxWidth: '100%', justifyContent: 'flex-end'}}>
        {tags.map((tag) => (
          <Chip
            key={tag}
            label={tag}
